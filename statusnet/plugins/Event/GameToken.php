 ...
 */
class GameToken extends Managed_DataObject
{
    const OBJECT_TYPE = 'http://activitystrea.ms/schema/1.0/event';

    public $__table = 'game_token'; // table name
    ...
    {
        return Memcached_DataObject::staticGet('GameToken', $k, $v);
    }
...
'nickname' => array('type' => 'varchar', 'length' => 64),
                'date' => array('type' => 'string', 'not null' => true),
                'token' => array('type' => 'int', 'not null' => true),
            ),
  ...
  static function getToken($profile_id)
    {
        return GameToken::staticGet('profile_id', $profile_id);
    }
